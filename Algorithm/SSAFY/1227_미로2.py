...
def solution(_board):
    start_y, start_x = map(int, [[i, j] for i in range(100) for j in range(100) if _board[i][j] == 2][0])
    dest_y, dest_x = map(int, [[i, j] for i in range(100) for j in range(100) if _board[i][j] == 3][0])
    ...
    while len(queue) != 0:
        ...
        for i in range(4):
            ...
            if -1 < next_y < 100 and -1 < next_x < 100:
